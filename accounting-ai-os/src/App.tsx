...
import { LayoutDashboard, FileText, Cpu, Monitor, Settings } from 'lucide-react';
  ...
  return (
    <Float speed={1.5} rotationIntensity={0.5} floatIntensity={1}>
      <mesh position={position}>
        ...
        <Text position={[0, 0, 0.06]} fontSize={0.2} color={color}>
          {text}
        </Text>
      </mesh>
    </Float>
    ...
    <div className="h-screen w-screen bg-black text-cyan-400 overflow-hidden font-mono relative">
      ...
      <div className="absolute inset-0 z-10 pointer-events-none flex flex-col justify-between p-8">
        <header className="flex justify-between items-center pointer-events-auto">
          <motion.div initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="bg-cyan-900/30 backdrop-blur-md border border-cyan-500/50 p-4 rounded-br-3xl rounded-tl-xl">
        ...
        <div className="flex justify-between items-end h-full">
          <motion.div initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="pointer-events-auto space-y-4 bg-cyan-900/20 backdrop-blur-lg border border-cyan-500/30 p-6 rounded-tr-3xl rounded-bl-xl">
            ...
            <button onClick={() => executeAction('open_calculator')} className="w-full flex items-center gap-3 p-3 bg-cyan-500/10 border border-cyan-500/50 rounded-lg hover:bg-cyan-500/30 transition-all group">
              <Monitor size={18} /> <span className="text-sm">Launch Calculator</span>
            </button>
            <button onClick={() => executeAction('open_notepad')} className="w-full flex items-center gap-3 p-3 bg-cyan-500/10 border border-cyan-500/50 rounded-lg hover:bg-cyan-500/30 transition-all group">
              <FileText size={18} /> <span className="text-sm">Open Notepad</span>
            </button>
            <button onClick={() => executeAction('open_explorer')} className="w-full flex items-center gap-3 p-3 bg-cyan-500/10 border border-cyan-500/50 rounded-lg hover:bg-cyan-500/30 transition-all group">
              <LayoutDashboard size={18} /> <span className="text-sm">File Explorer</span>
            </button>
          </motion.div>

          <motion.div initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="pointer-events-auto space-y-4 bg-cyan-900/20 backdrop-blur-lg border border-cyan-500/30 p-6 rounded-tl-3xl rounded-br-xl">
            <h3 className="text-xs font-bold uppercase opacity-50 mb-4">AI Agency Status</h3>
            <div className="space-y-3">
              <div className="flex items-center gap-3 text-sm"><div className="w-2 h-2 rounded-full bg-green-500 animate-ping" /><span>Collector: Idle</span></div>
              <div className="flex items-center gap-3 text-sm"><div className="w-2 h-2 rounded-full bg-yellow-500 animate-pulse" /><span>Auditor: Processing</span></div>
              <div className="flex items-center gap-3 text-sm"><div className="w-2 h-2 rounded-full bg-cyan-500 animate-bounce" /><span>Analyst: Ready</span></div>
            </div>
